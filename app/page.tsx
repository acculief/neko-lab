    ...
    <div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {CATEGORIES.map(cat => (
              <Link
                ...
                className="bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm font-bold transition-all border border-white/20 hover:border-[#C4892A]/50"
              >
                {cat.icon} {cat.name}
              </Link>
            ))}
      ...
      <section className="bg-[#FFF8ED] border-y border-[#E8C87A] py-6 px-4">
        <div className="max-w-3xl mx-auto flex flex-col md:flex-row items-center gap-6">
          <div className="flex-shrink-0 w-14 h-14 rounded-full flex items-center justify-center text-2xl shadow-md"
      ...
      <section className="max-w-5xl mx-auto px-4 py-12">
        <h2 className="text-2xl font-black text-[#1A2D4A] mb-8 text-center">
          カテゴリから探す
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {CATEGORIES.map(cat => (
            <Link
              key={cat.id}
              href={`/category/${cat.slug}`}
              className="group bg-white rounded-2xl p-4 text-center shadow-sm hover:shadow-md transition-all border border-[#E8E3DC] hover:border-[#C4892A]"
            >
      ...
      <section className="bg-white border-y border-[#E8E3DC] py-12 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-black text-[#1A2D4A]">
              🍖 プレミアムキャットフード スコアTOP3
            </h2>
            <Link href="/category/premium-cat-food" className="text-sm text-[#C4892A] font-bold hover:underline">
              全て見る →
            </Link>
          </div>
      ...
      {/* スコア評価基準 */}
      <section className="max-w-5xl mx-auto px-4 py-12">
        <h2 className="text-2xl font-black text-[#1A2D4A] mb-6 text-center">
          スコアの評価基準
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            {
              tier: '🥇 ゴールド（90点以上）',
              bg: 'linear-gradient(135deg, #FFF8ED, #FFF3DC)',
              border: '#E8C87A',
              textColor: '#7A5420',
              desc: '最高品質。迷ったらこれ。素材・栄養・嗜好性すべてが高水準。'
            },
            {
              tier: '🟢 グリーン（75〜89点）',
              bg: 'linear-gradient(135deg, #F0F8F4, #E8F4EC)',
              border: '#A8D4B8',
              textColor: '#2A6040',
              desc: '高品質でバランス良し。コスパを重視する方にもおすすめ。'
            },
            {
              tier: '⬜ スタンダード（〜74点）',
              bg: 'linear-gradient(135deg, #F8F8F8, #F0F0F0)',
              border: '#D0D0D0',
              textColor: '#4A4A4A',
              desc: '標準的な品質。用途・予算によっては選択肢になりえる。'
            },
          ].map(item => (
            <div key={item.tier}
            ...
            >
              <p className="font-black mb-2" style={{ color: item.textColor }}>{item.tier}</p>
