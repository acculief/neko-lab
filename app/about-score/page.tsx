...
const CATEGORY_WEIGHTS: Record<string, { axes: string[]; note: string }> = {
  ...
  'おもちゃ': {
    axes: ['楽しさ×30%', '耐久性×20%', 'コスパ×25%', '口コミ×25%'],
    note: '猫が実際に遊んでくれるか（楽しさ・エンゲージメント）を最重視。安全性・耐久性・コスパも総合評価。',
  },
  'キャリー': {
    axes: ['携帯性×30%', '安全性×25%', '使いやすさ×25%', 'コスパ×20%'],
    note: '移動しやすさ・猫へのストレス軽減・出し入れしやすさを重点評価。機内対応や折りたたみ機能も加味。',
  },
  'ペット保険': {
